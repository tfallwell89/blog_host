...
| Recipe UI          | `apps/foodblog/src/components/recipe/`                     | Document model, canvas, editor shell, group panel          | `recipe-document.ts`                               |
...
| ----------------------------------------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Recipe editor chrome (action bar, save, view toggles) | `apps/foodblog/src/components/recipe/recipe-editor.tsx`                      | `apps/foodblog/src/styles/editor.css`, `apps/foodblog/src/lib/recipes/actions.ts`                                                                       |
...
| Recipe groups (the editor's right-hand rail)          | `apps/foodblog/src/components/recipe/group-panel.tsx`                        | `apps/foodblog/src/lib/recipes/queries.ts` (`getGroupsForBlog`), `apps/foodblog/src/lib/recipes/persistence.ts`, `apps/foodblog/src/styles/editor.css`  |
| "More in <group>" on a published recipe               | `apps/foodblog/src/components/recipe/recipe-page.tsx` (`RecipeRelated`)      | `apps/foodblog/src/lib/recipes/queries.ts` (`getRelatedPublishedRecipes`), `apps/foodblog/src/styles/site.css`                                          |
...
| Split view (live preview beside the canvas)           | `apps/foodblog/src/components/recipe/recipe-editor.tsx` (`view` state)       | `apps/foodblog/src/styles/editor.css`, `apps/foodblog/src/styles/site.css` (`@container site`), `apps/foodblog/src/lib/use-media-query.ts`              |
...
| **Group**             | `Group` — a named set of related _recipes_ ("Weeknight dinners"), joined through `GroupedRecipe`. Created by typing a name in the editor's group panel; readers meet it as "More in …" |
| **Group panel**       | The rail to the right of the canvas where a recipe's groups are added and the recipes already in them are listed                                                                       |
| **Ingredient group**  | `IngredientGroup` — an optionally titled, ordered list of ingredients ("For the sauce"). A section _inside_ one recipe, unrelated to `Group`                                           |
...
| Group writes                  | The same transaction upserts `Group` rows by slug, rebuilds `GroupedRecipe`, and deletes groups left empty — a wrong `blogId` would leak or delete another tenant's groups | `apps/foodblog/src/lib/recipes/persistence.ts` (`groupIdsFor`, `pruneEmptyGroups`)                                           |
...
| Recipe groups and related recipes           | Implemented             | `apps/foodblog/src/components/recipe/group-panel.tsx`   | Named in the editor rail, read as "More in …"; no group archive page yet   |
